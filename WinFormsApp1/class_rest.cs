...
namespace EasyREST
{
    ...
    /// <summary>
    /// Class encapsulating a list of nodes (class rest)
    /// and methods to load, save and edit the list
    /// </summary>
    public class list_items
    {
        public int Count { get => items.Count;  } // Number of nodes in list
        public string Title { get; set; } = "";
        public string Description { get; set; } = "";

        public int Port { get; set; } = 8080; // Port to listen on

        private List<class_rest> items = [];  // The nodes list
        ...
        /// <summary>
        /// Update noun and description of a node
        /// </summary>
        /// <param name="ID"></param>   ID of the node
        /// <param name="item"></param> The node to copy details from
        /// <returns></returns>
        ...
        /// <summary>
        /// Remove all nodes, set defaults for project title,
        /// description, port and start ID
        /// </summary>
        ...
        public string Save(string filename)
        {
            if (filename != "")
            {
                try
                {
                    // JSON serialize the project and save it
        ...
        /// <summary>
        /// Load project from a jsopn encoded file
        /// </summary>
        ...
        public string Load(string filename)
        {
            ...
            if (filename != "")
            {
                try
                {
                    ...
                    {
                        // Try to load file into a temporary list
                        string json = r.ReadToEnd();
                        list_items tmp = JsonSerializer.Deserialize<list_items>(json)!;
                        // Save project title, description and listening port
    ...
    /// <summary>
    /// Simple ID generator, increments an internal count each time it is used
    /// </summary>
    internal static class IDs
    {
        ...
        /// Set the start ID
        /// </summary>
        /// <param name="startID">The start ID to start counting from</param>
